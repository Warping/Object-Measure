...
ap.add_argument("-w", "--width", type=float, required=True,
    help="width of the reference object in inches")
...
while True:
    ...
    for c in cnts:
        ...
        # --- NEW FILTER: at least one side must be >= 0.5 inches ---
    ...
    # --- Find reference object (bottom-left) ---
    min_x = float('inf')
    max_y = float('-inf')
    reference_idx = 0
    ...
    for idx, obj in enumerate(objects):
        box = obj["box"]
        bl_x, bl_y = box[3]
        if (bl_x < min_x) or (bl_x == min_x and bl_y > max_y):
            min_x = bl_x
            max_y = bl_y
            reference_idx = idx
